...
// +
  public getNews(perPageCount: number, pageNumber): Observable<News[]> {
    return this.http.get<News[]>(`${this.mainUrl}/all`, {
      // @ts-ignore
      params: {
...
// +
  public getImagesOfNews(id: number): Observable<NewsImage[]> {
    return this.http.get<NewsImage[]>( `${this.mainUrl}/images/${id}`).pipe(
      map(data => data.map(entryData => new NewsImage().deserialize(entryData)))
    );
  }
// +
